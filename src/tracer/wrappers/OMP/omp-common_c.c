...
/* Obtain @ for omp_get_thread_num_real */
	...
	omp_set_num_threads_real =
		(void(*)(int)) __real_omp_set_num_threads;

	/* Obtain @ for omp_set_lock */
	omp_set_lock_real =
		(void(*)(omp_lock_t*)) __real_omp_set_lock;

	/* Obtain @ for omp_unset_lock */
	omp_unset_lock_real =
		(void(*)(omp_lock_t*)) __real_omp_unset_lock;
...
void
__wrap_omp_set_num_threads(int num_threads)
{
#if defined(DEBUG)
	fprintf(stderr, PACKAGE_NAME
...
void __wrap_omp_unset_lock (omp_lock_t *lock)
{
	void *lock_ptr = (void *)lock;
