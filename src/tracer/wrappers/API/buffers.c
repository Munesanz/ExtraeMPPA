...
Buffer_t * new_Buffer (int n_events, char *file, int enable_cache)
{
	...
	assert(buffer->FirstEvt && "Allocation failed\n");
...
#if defined(ARCH_SPARC64) || defined(OS_RTEMS) || defined(OS_CLUSTER)
	ssize_t r;
#endif
...
#else /* ARCH_SPARC64 */
	r = fwrite ( head, 1,  buffer->FillCount*sizeof(event_t), buffer->fd);
