...
int __Extrae_Utils_rename_or_copy (char *origen, char *desti)
{
   ...
   source = fopen(origen, "rb");
   if (source == NULL) {
    /* handle error */
    perror("file open for reading");
    exit(EXIT_FAILURE);
   }
   target = fopen(desti, "wb");
   if (desti == NULL) {
     /* handle error */
   ...
   if (fclose(source)) perror("close output file");
   if (fclose(target)) perror("close input file");
   if( remove(origen) !=0)
	perror("delete");
...
//# error "Don't know how to check whether a directory exists"
	printf("__Extrae_Utils_directory_exists \n");
#endif
	return 1;
}
